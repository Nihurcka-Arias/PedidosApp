...
namespace CapaDatos
{
    public class DArticulo : DbConnection
    {
        ...
        public DataTable BuscarNombreODCodigo(DArticulo Articulo)
        {
            DataTable DtResultado = new DataTable("articulo");
            using (SqlConnection connection = GetConnection())
            {
                ...
                using (SqlCommand command = new SqlCommand())
                {
                    ...
                    {
                        command.Connection = connection;
                        command.CommandText = "spbuscar_articulo_nombre_o_codigo"; // Cambié el nombre del procedimiento
                        command.CommandType = CommandType.StoredProcedure;
                        ...
                        ParTextoBuscar.SqlDbType = SqlDbType.VarChar;
                        ParTextoBuscar.Size = 50;
                        ParTextoBuscar.Value = Articulo.TextoBuscar; // Asumo que esta propiedad existe en tu entidad
                        command.Parameters.Add(ParTextoBuscar);
